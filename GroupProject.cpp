...
int lastID = 0;
BinaryTreeTemplate<string> customerlist;//Using Binary Search
int main() {
	// variable
	...
	customerlist.displayInOrder();//Using Dynamic Queue display method

	string temp;
	cout << "Name to search? : "; //Doing Binary Search
	cin >> temp;
	if (customerlist.serarchNode(temp))
...
void loadCustomerData(customer c[]) {
	...
	while (dataIn >> name) {
		...
		customerlist.insertNode(name);//Using Binary InsertNode
